...
import { TARSAgentModel } from 'src/agent/agent.types';

const geminiApiKey = process.env.GEMINI_API_KEY;
const anthropicApiKey = process.env.ANTHROPIC_API_KEY;
const openaiApiKey = process.env.OPENAI_API_KEY;

const proxyUrl = process.env.TARS_LLM_PROXY_URL;
  ...
  async getModels() {
    if (proxyUrl) {
      try {
        ...
        // Map proxy response to TARSAgentModel format
        const models: TARSAgentModel[] = proxyModels.data.map(
          (model: any) => ({
            provider: 'proxy',
