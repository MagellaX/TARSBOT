...
@Injectable()
export class ProxyService implements TARSAgentService {
  private readonly openai: OpenAI;
  private readonly logger = new Logger(ProxyService.name);

  constructor(private readonly configService: ConfigService) {
    const proxyUrl = this.configService.get<string>('TARS_LLM_PROXY_URL');

    if (!proxyUrl) {
      this.logger.warn(
        'TARS_LLM_PROXY_URL is not set. ProxyService will not work properly.',
      );
    }
  ...
  /**
   * Convert TARS messages to Chat Completion format
   */
