...
// Backend URLs
const TARS_AGENT_BASE_URL = process.env.TARS_AGENT_BASE_URL;
const TARS_DESKTOP_VNC_URL = process.env.TARS_DESKTOP_VNC_URL;
...
app
  ...
  .then(() => {
    ...
    expressApp.use("/api/proxy/websockify", (req, res) => {
      ...
      // Rewrite path
      const targetUrl = new URL(TARS_DESKTOP_VNC_URL!);
      req.url =
        targetUrl.pathname +
