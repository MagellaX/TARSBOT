...
export const TopicPopover: React.FC<TopicPopoverProps> = ({
  ...
  const modifiedChildren = React.Children.map(children, (child) => {
    ...
    // Replace text-tars-bronze-light-11 with text-tars-bronze-light-12 when open
    const updatedClassName = isOpen 
      ? existingClassName.replace('text-tars-bronze-light-11', 'text-tars-bronze-light-12')
      : existingClassName;
  ...
  return (
    <div className="relative" ref={popoverRef}>
      <div onClick={handleToggle} className={isOpen ? "bg-tars-bronze-light-1 rounded-full" : ""}>
        {modifiedChildren}
      </div>
    </div>
  );
