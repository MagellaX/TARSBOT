...
description: "Use any LLM provider with TARS through LiteLLM proxy"
---

# Connect Any LLM to TARS with LiteLLM

LiteLLM acts as a unified proxy that lets you use 100+ LLM providers with TARS - including Azure OpenAI, AWS Bedrock, Anthropic, Hugging Face, Ollama, and more. This guide shows you how to set up LiteLLM with TARS.
...
## Quick Start with TARS's Built-in LiteLLM Proxy

TARS includes a pre-configured LiteLLM proxy service that makes it easy to use any LLM provider. Here's how to set it up:
...
<Steps>
  <Step title="Use Docker Compose with Proxy">
    ...
    # Start TARS with LiteLLM proxy
    docker-compose -f docker/docker-compose.proxy.yml up -d
    ```
    
    This automatically:
    - Starts the `tars-llm-proxy` service on port 4000
    - Configures the agent to use the proxy via `TARS_LLM_PROXY_URL`
  ...
  <Step title="Customize Model Configuration">
    ...
    ```yaml
    # packages/tars-llm-proxy/litellm-config.yaml
    model_list:
      # Add Azure OpenAI
  ...
  <Step title="Verify Models are Available">
    The TARS agent automatically queries the proxy for available models:
    
    ```bash
    # Check available models through TARS API
...
```mermaid
graph LR
    A[TARS UI] -->|Select Model| B[TARS Agent]
    B -->|TARS_LLM_PROXY_URL| C[LiteLLM Proxy :4000]
...
1. **tars-llm-proxy Service**: A LiteLLM instance running in Docker that:
   - Runs on port 4000 within the TARS network
   - Uses the config from `packages/tars-llm-proxy/litellm-config.yaml`
   - Inherits API keys from environment variables

2. **Agent Integration**: The TARS agent:
   - Checks for `TARS_LLM_PROXY_URL` environment variable
...
# Use "smart-model" in TARS config
```
...
services:
  tars-agent:
    environment:
      ...
      - TARS_LLM_PROXY_URL=http://your-litellm-server:4000
    # ... rest of config
```
...
export TARS_LLM_PROXY_URL=http://your-litellm-server:4000
...
# Then start TARS with:
export TARS_LLM_PROXY_URL=http://localhost:4000
docker-compose up -d
```
...
<AccordionGroup>
  ...
  <Accordion title="Authentication errors">
    Verify master key in both LiteLLM and TARS:
    ```bash
    # Test LiteLLM
...
<Note>
  **Pro tip:** Start with a single provider, then add more as needed. LiteLLM makes it easy to switch or combine models without changing TARS configuration.
</Note>