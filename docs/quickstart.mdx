...
TARS can be deployed in several ways depending on your needs:
...
<Steps>
  <Step title="Visit the Template">
    Click the Deploy Now button in the TARS template on Railway.
  </Step>
  <Step title="Add Anthropic Key">
    Enter either your `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or `GEMINI_API_KEY` for the tars-agent resource.
  </Step>
...
<Step title="Start the Agent Stack">
  ...
  # Optional: override the image namespace if you pushed to a different account
  # export TARS_IMAGE_REGISTRY=ghcr.io/your-gh-username
  # Optional: remap host ports if 9990/9992 are already in use
  # export TARS_DESKTOP_HOST_PORT=10990
  # export TARS_UI_HOST_PORT=10992
  # export TARS_POSTGRES_HOST_PORT=15432
  ```

This starts all four services:

- **TARS Desktop**: Containerized Linux environment
...
Navigate to [http://localhost:9992](http://localhost:9992) to access the TARS UI.

**Two ways to interact:**
1. **Tasks**: Enter task descriptions to have TARS work autonomously
...
<Note>
  **🔐 Password Manager Support**: TARS can handle authentication automatically when you install a password manager extension. See our [password management guide](/guides/password-management) for setup instructions.
</Note>
...
Now let's see TARS in action! Try these example tasks:
...
# Just the agent
docker-compose -f docker/docker-compose.yml logs -f tars-agent
```
...
curl -X POST http://localhost:9990/computer-use \
  -H "Content-Type: application/json" \
  -d '{"action": "type_text", "text": "Hello, TARS!"}'
```
...
<AccordionGroup>
  ...
  <Accordion title="Agent errors or no response">
    ...
    ```bash 
    cat docker/.env
    docker-compose -f docker/docker-compose.yml logs tars-agent 
    ```
...
<CardGroup cols={2}>
  ...
  <Card title="Takeover Mode" icon="hand" href="/guides/takeover-mode">
    Take control when you need to guide TARS
  </Card>
  ...
  <Card title="API Integration" icon="code" href="/api-reference/introduction">
    Automate TARS with your applications
  </Card>
</CardGroup>
