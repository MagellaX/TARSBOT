---
title: 'Task UI'
description: 'Documentation for the TARS Task UI'
---

## TARS Task UI

The TARS Task UI provides a web-based interface for interacting with the TARS agent system. It combines a action feed with an embedded noVNC viewer, allowing you to watch it perform task on the desktop in real-time.

<img src="/static/chat-ui-overview.png" alt="TARS Task Detail" className="w-full max-w-4xl" />

## Accessing the UI

When running the full TARS agent system, the Task UI is available at:
...
The task interface displays different types of messages based on TARS's content block structure:
...
The TARS Task UI is built with:
...
1. Ensure all TARS services are running
...
1. Ensure the TARS container is running
