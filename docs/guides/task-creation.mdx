...
# Creating and Managing Tasks in TARS

This guide will walk you through everything you need to know about creating tasks that TARS can execute effectively, and managing them through their lifecycle.

## Understanding Tasks

A task is any job you want TARS to complete. Tasks can be:
...
TARS has powerful file handling capabilities that make it perfect for document processing and data analysis tasks.
...
- **Document Processing**: Upload PDFs, spreadsheets, or documents for TARS to analyze
...
<Note>
  **Game Changer**: TARS can read entire files, including PDFs, directly into the LLM context. This means it can process large amounts of data quickly and understand complex documents without manual extraction.
</Note>
...
<Steps>
  ...
  <Step title="Start Task">
    Press the arrow button or hit Enter. TARS will start loading and begin working on your task.
  </Step>
</Steps>
...
(TARS handles all authentication including 2FA automatically via password manager)
```
...
Watch TARS work through the task detail viewer:

- **Green dot**: Task is actively running
- **Status messages**: Current step being executed
- **Desktop view**: See what TARS sees in real-time

#### Chat Messages

TARS provides updates like:
```
...
(Note: TARS handles all authentication including 2FA automatically using the built-in password manager)
...
After each task:
1. Review the approach TARS took
...
<Note>
  **Pro Tip**: Start with simple tasks to understand TARS's capabilities, then gradually increase complexity as you learn what works best.
</Note>